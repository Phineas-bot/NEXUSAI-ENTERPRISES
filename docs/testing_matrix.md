...
| Load/Performance | Validate up to 5K concurrent clients & 50 GB uploads | Resumable uploads, replica fan-out, manifest finalization | K6/Locust profiles in `load_tests/` (see below) with Grafana synthetic dashboards |
...
- **Load/Perf:** create K6 or Locust profiles in `load_tests/` (skeleton below). Capture metrics (p95 latency, throughput, error budget) and push to Grafana dashboards identified in Section 6.
- **Chaos:** staging-only scripts (see `docs/staging_plan.md`). Automation should open issues when SLA breaches occur.

## Skeleton Load Test Suite

```
load_tests/
└── README.md              # instructions + metrics
└── k6/
    └── resumable_upload.js
└── locust/
    └── resiliency.py
...
The actual scripts are future work; this matrix defines expectations so we can track progress.
